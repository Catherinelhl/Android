...
//        data.scatterData = generateScatterData()
    ...
    func generateLineData() -> LineChartData {
        let entries = (0..<ITEM_COUNT).map { (i) -> ChartDataEntry in
            return ChartDataEntry(x: Double(i) + 0.5, y: Double.random(in: 50...80))
        }
        let entries2 = (0..<ITEM_COUNT).map { (i) -> ChartDataEntry in
            return ChartDataEntry(x: Double(i) + 0.5, y: Double.random(in: 50...80))
        }
        let entries3 = (0..<ITEM_COUNT).map { (i) -> ChartDataEntry in
            return ChartDataEntry(x: Double(i) + 0.5, y: Double.random(in: 50...80))
        }
        ...
        let set2 = LineChartDataSet(values: entries2, label: "Line2 DataSet")
        set2.setColor(.blue)
        set2.lineWidth = 1
        set2.drawCirclesEnabled = false
        set2.drawValuesEnabled = false
        
        let set3 = LineChartDataSet(values: entries3, label: "Line3 DataSet")
        set3.setColor(.green)
        set3.lineWidth = 1
